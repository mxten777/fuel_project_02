// Moved to ../molecules/BusinessMenuModal.tsx
const menuData = [
  {
...
const BusinessMenuModal = ({ open, onClose }: { open: boolean; onClose: () => void }) => {
  return (
    <Modal open={open} onClose={onClose}>
      <h2 className="text-xl xs:text-2xl sm:text-3xl font-bold mb-4 xs:mb-6 sm:mb-10 text-center text-navy dark:text-white drop-shadow">업무 메뉴</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 xs:gap-6 sm:gap-8 w-full">
        {menuData.map((menu, idx) => (
          <div key={idx} className={`rounded-2xl p-2 xs:p-4 sm:p-6 shadow-lg border border-blue-100 dark:border-navy/40 flex flex-col items-center ${menu.color} transition-all hover:scale-[1.03]`}>
            <div className="font-bold mb-2 xs:mb-3 sm:mb-4 text-base xs:text-lg sm:text-xl text-navy dark:text-white text-center drop-shadow">{menu.title}</div>
            <ul className="space-y-1 xs:space-y-2 w-full">
              {menu.items.map((item, i) => (
                <li key={i}>
                  <button className="w-full text-center px-2 xs:px-3 py-1 xs:py-2 rounded-lg bg-white/70 dark:bg-navy/60 text-navy dark:text-white shadow hover:bg-blue-100 dark:hover:bg-blue-900 transition font-medium border border-blue-50 dark:border-navy/30 text-xs xs:text-sm sm:text-base">
                    {item}
                  </button>
                </li>
              ))}
