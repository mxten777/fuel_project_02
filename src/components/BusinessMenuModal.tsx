...
const BusinessMenuModal = ({ open, onClose }: { open: boolean; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
      <div className="bg-white/90 dark:bg-navy-dark rounded-2xl p-8 max-w-5xl w-full relative overflow-y-auto max-h-[90vh] shadow-2xl flex flex-col items-center">
        <button className="absolute top-4 right-6 text-3xl text-navy dark:text-white hover:text-blue-500" onClick={onClose}>×</button>
        <h2 className="text-3xl font-bold mb-10 text-center text-navy dark:text-white drop-shadow">업무 메뉴</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
          {menuData.map((menu, idx) => (
            <div key={idx} className={`rounded-2xl p-6 shadow-lg border border-blue-100 dark:border-navy/40 flex flex-col items-center ${menu.color} transition-all hover:scale-[1.03]`}>
              <div className="font-bold mb-4 text-lg text-navy dark:text-white text-center drop-shadow">{menu.title}</div>
              <ul className="space-y-2 w-full">
                {menu.items.map((item, i) => (
                  <li key={i}>
                    <button className="w-full text-center px-3 py-2 rounded-lg bg-white/70 dark:bg-navy/60 text-navy dark:text-white shadow hover:bg-blue-100 dark:hover:bg-blue-900 transition font-medium border border-blue-50 dark:border-navy/30">
                      {item}
                    </button>
                  </li>
                ))}
