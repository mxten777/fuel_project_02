...
export default function KPIDashboard() {
  return (
  <div className="w-full max-w-3xl mx-auto p-2 xs:p-3 sm:p-6 md:p-8 bg-white/90 dark:bg-navy/95 rounded-3xl shadow-2xl mt-2 xs:mt-3 sm:mt-8">
  <h2 className="text-xl xs:text-2xl sm:text-3xl font-bold text-center mb-3 xs:mb-4 sm:mb-8 text-navy dark:text-white drop-shadow-lg">KPI 달성 비교</h2>
      {/* KPI 비교 표 (상단) */}
      <div className="mb-4 xs:mb-6 sm:mb-10">
        <h3 className="text-base xs:text-lg sm:text-xl font-bold text-navy dark:text-white mb-1 xs:mb-2 sm:mb-4">KPI 비교표</h3>
        <div className="overflow-x-auto">
          <table className="min-w-[380px] xs:min-w-[440px] w-full border border-navy-light rounded-xl bg-white dark:bg-navy-light text-center text-[11px] xs:text-xs sm:text-base">
            <thead>
            ...
            <tbody>
              {kpiTableData.map((kpi, idx) => {
                ...
                return (
                  <tr key={kpi.name} className={"border-b border-navy-light " + (idx === kpiTableData.length-1 ? "" : "") }>
                    <td className="px-1 xs:px-2 sm:px-4 py-1 xs:py-2 font-bold whitespace-nowrap">{kpi.name}</td>
                    <td className="px-1 xs:px-2 sm:px-4 py-1 xs:py-2 whitespace-nowrap">{kpi.value}{kpi.unit}</td>
                    <td className="px-1 xs:px-2 sm:px-4 py-1 xs:py-2 whitespace-nowrap">{kpi.target}{kpi.unit}</td>
                    <td className="px-1 xs:px-2 sm:px-4 py-1 xs:py-2 whitespace-nowrap">{kpi.prev}{kpi.unit}</td>
                    <td className={"px-1 xs:px-2 sm:px-4 py-1 xs:py-2 font-semibold whitespace-nowrap " + (diff > 0 ? "text-green-600" : diff < 0 ? "text-red-600" : "")}>{diff > 0 ? `+${diff} (${diffRate}%)` : `${diff} (${diffRate}%)`}</td>
                    <td className="px-1 xs:px-2 sm:px-4 py-1 xs:py-2 font-semibold whitespace-nowrap">{percent}%</td>
                    <td className="px-1 xs:px-2 sm:px-4 py-1 xs:py-2 whitespace-nowrap">{kpi.value >= kpi.target ? <span className="text-green-600 font-bold">달성</span> : <span className="text-yellow-600 font-bold">진행중</span>}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {/* KPI 카드 내용 (하단) */}
  <div className="grid grid-cols-1 gap-3 xs:gap-4 sm:grid-cols-2 sm:gap-8">
        ...
        {kpiData.map((kpi) => {
          ...
          return (
            <div key={kpi.name} className="bg-white dark:bg-navy-light rounded-xl p-3 xs:p-4 sm:p-8 shadow-xl flex flex-col relative overflow-visible min-h-[180px] xs:min-h-[220px] sm:min-h-[340px]">
